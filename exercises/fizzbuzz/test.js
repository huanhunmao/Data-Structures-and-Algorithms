...
test('Calling fizzbuzz with 15 prints out the correct values', () => {
  ...
  expect(console.log.mock.calls[2][0]).toEqual('fizz'); // 3 
  expect(console.log.mock.calls[3][0]).toEqual(4);
  expect(console.log.mock.calls[4][0]).toEqual('buzz'); // 5
  expect(console.log.mock.calls[5][0]).toEqual('fizz'); // 6
  expect(console.log.mock.calls[6][0]).toEqual(7);
  expect(console.log.mock.calls[7][0]).toEqual(8);
  expect(console.log.mock.calls[8][0]).toEqual('fizz'); // 9
  expect(console.log.mock.calls[9][0]).toEqual('buzz'); // 10
  expect(console.log.mock.calls[10][0]).toEqual(11);
  expect(console.log.mock.calls[11][0]).toEqual('fizz'); // 12
  expect(console.log.mock.calls[12][0]).toEqual(13);
  expect(console.log.mock.calls[13][0]).toEqual(14);
  expect(console.log.mock.calls[14][0]).toEqual('fizzbuzz'); // 15
});
